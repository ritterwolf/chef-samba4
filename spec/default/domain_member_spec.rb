...
describe 'samba4::domain_member' do
  context 'on Centos 7.1' do
    ...
    before do
      ...
      stub_command('echo | net ads testjoin').and_return(false)
    end
    ...
    it 'joins the domain' do
      expect(chef_run).to run_execute('join-domain')
        .with(command: 'net ads join -U Administrator%Administr8')
    end
  end
